...
df<-read.csv(fname,sep="\t",stringsAsFactors=F)

df$FECHA<-as.POSIXlt(df$FECHA)
df$HORA<-as.hms(df$HORA)

save(file='./data/DFP1.RData',list=c("df"))
```
...
```{r}
# dim, str, ...
print(str(BreadBasket_DMS))

print(head(BreadBasket_DMS,10))

print(tail(BreadBasket_DMS,10))

print(summary(BreadBasket_DMS))
```
...
PsychBike<-read_sav(filename)

save(file="data/PsychBike.Rdata",PsychBike)
```
...
print(head(PsychBike,10))

print(tail(PsychBike,10))

print(summary(PsychBike))
...
FileCodificado<-read_json("data/FileCodificado.json",simplifyVector = TRUE)

FileCodificado_file<-read_file("data/FileCodificado.json")

FileCodificado_file<-fromJSON(FileCodificado_file)

save(file="data/FileCodificado.Rdata",FileCodificado)
```
...
```{r}
# str, head, tail, summary
print(str(FileCodificado))

print(head(FileCodificado,10))

print(tail(FileCodificado,10))

print(summary(FileCodificado))
...
```{r}
# ...
write_json(iris, path = "./data/iris.json", pretty = FALSE)
...
ERCA <- read_excel(
  "./data/ERCA.xlsx",
  col_types = c("date", "text", "numeric", "numeric", "text"),
)

ERCA <- read_excel("data/ERCA.xlsx", sheet = "Hoja1 - Tabla 1")

ERCA<-ERCA[-which(ERCA[,1] == "IDENTIFICA"),]
...
```{r}
# save()
save(file="data/ERCA.RData",ERCA)
```
...
```
  ...
  3. En lugar de un bucle for usa `lapply`
      ...
      -  La lista devuelta por `lapply` no tiene nombres. Podemos nombrar esta lista con `names(x) <- sheets`, donde `x` es la lista obtenida con `lapply`, y `sheets` los nombres de las hojas obtenidas con excel_sheets`.
...
#1.
measurments<-read_excel(filename, sheet="measurments")
neurophaty<-read_excel(filename, sheet="neuropathy")
tuning_fork_NSS<-read_excel(filename, sheet="tuning fork NSS")
mobility<-read_excel(filename, sheet="mobility")
Durometer<-read_excel(filename, sheet="Durometer")
...
for (i in sheets){
  df[[i]]<-read_excel(filename, sheet=i)
}


#4.

df_lapply<-lapply(sheets,function(sheets){read_excel(filename, sheet=sheets)})

names(df_lapply)<-sheets


save(file='./data/df_lapply.RData',list=c("df_lapply"))
```
...
```{r}
# str, head, tail, summary
str(df_lapply,max.level = 10)
head(df_lapply, max.level=5)
tail(df_lapply, max.level=20)
summary(df_lapply, max.level=1)
